...
public class Boss3 extends Creature
{
    ...
    public Boss3(Handler handler, float x, float y, int health, int id)
    {
        super(handler, x, y, 128, 128, maxHealth,id, "Boss3");
        this.id = id;
        attackCooldown = 350L;
        startHealth = health;
        this.health = health;
    ...
    public void tick()
    {
        ...
        if(handler.getWorld().getEntityManager().getPlayer().getKilledBosses() > 2)
            suicide();
    ...
    public void die()
    {
        for(int i = 0; i < 5; i++) {
            ...
            handler.getWorld().getItemManager().addItem(Item.gunPowder.createNew((int)x - 2, (int)y, false, false, false));
            handler.getWorld().getItemManager().addItem(Item.gunPowder.createNew((int)x - 2, (int)y, false, false, false));
            handler.getWorld().getItemManager().addItem(Item.ironBarItem.createNew((int)x - 2, (int)y, false, false, false));
            handler.getWorld().getItemManager().addItem(Item.steelBarItem.createNew((int)x - 2, (int)y, false, false, false));
        }
        handler.getWorld().getEntityManager().getPlayer().setKilledBosses(3);
    }
